...
def generate_sprite(
    ...
    try:
        ...
        n = (
            list(game.species["species"]).index(cat.species)
        ) + 1  # add 1 because people don't count from 0 smh
        ...
        if cat.pelt.name not in ["Tortie", "Calico"]:
            ...
            new_sprite.blit(
                sprites.sprites[
                    cat.pelt.tortiebase + f"{n}_" + cat.pelt.colour + cat_sprite
                ],
                (0, 0),
            )
        ...
        if cat.pelt.points:
            points = sprites.sprites[
                "white" + f"{n}_" + cat.pelt.points + cat_sprite
            ].copy()
            if (
                cat.pelt.white_patches_tint != "none"
                and cat.pelt.white_patches_tint
        ...
        if cat.pelt.vitiligo:
            new_sprite.blit(
                sprites.sprites["white" + f"{n}_" + cat.pelt.vitiligo + cat_sprite],
                (0, 0),
            )

        # draw eyes & scars1
        eyes = sprites.sprites[
            "eyes" + f"{n}_" + cat.pelt.eye_colour + cat_sprite
        ].copy()
        if cat.pelt.eye_colour2 != None:
            eyes.blit(
                sprites.sprites["eyes2" + f"{n}_" + cat.pelt.eye_colour2 + cat_sprite],
                (0, 0),
            )
        ...
        if not dead:
            ...
            new_sprite.blit(
                sprites.sprites["lineartdead" + f"{n}_" + cat_sprite], (0, 0)
            )
        # draw skin and scars2
        blendmode = pygame.BLEND_RGBA_MIN
        new_sprite.blit(
            sprites.sprites["skin" + f"{n}_" + cat.pelt.skin + cat_sprite], (0, 0)
        )
                ...
                for accessory in cat_accessories:
                    if accessory in getattr(Pelt, category):
                        if accessory in cat.pelt.plant_accessories:
                            new_sprite.blit(
                                sprites.sprites[
                                    "acc_herbs" + f"{n}_" + accessory + cat_sprite
                                ],
                                (0, 0),
                            )
                            ...
                            new_sprite.blit(
                                sprites.sprites[
                                    "acc_wild" + f"{n}_" + accessory + cat_sprite
                                ],
                                (0, 0),
                            )
                            ...
                            new_sprite.blit(
                                sprites.sprites[
                                    "collars" + f"{n}_" + accessory + cat_sprite
                                ],
                                (0, 0),
                            )
        ...
        if (
            ...
            if cat.status.group == CatGroup.DARK_FOREST:
                ...
                temp = sprites.sprites[
                    "fadestarclan" + f"{n}_" + stage + cat_sprite
                ].copy()
                temp.blit(new_sprite, (0, 0))
                new_sprite = temp
